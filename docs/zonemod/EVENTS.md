...
on("player.gainExp", function (p, v) {
  return v * 2.0;
});
on("player.expLoss", function (p, v) {
  return v * 0.5;
});

// +20% move speed for all players
on("player.moveSpeed", function (p, v) {
  return v * 1.2;
});

// Set max level to 80
on("player.maxLevel", function (p, v) {
  return 80;
});
```
...
on("player.login", function (player) {
  ...
  player.sendMessage(
    "[Server] Your level: " + player.level + " | Race: " + player.race,
  );
});
...
on("player.levelUp", function (player, newLevel) {
  if (newLevel >= 50) {
    GameServer.broadcast(
      "[Level Up] " + player.name + " reached level " + newLevel + "!",
    );
  }
});
```
...
on("player.chat", function (player, message) {
  if (message === "!online") {
    player.sendMessage(
      "[Server] Online players: " + GameServer.getPlayerCount(),
    );
...
on("monster.maxHP", function (p, hp) {
  return hp * 2;
});
on("monster.dropRate", function (p, rate) {
  return rate * 0.5;
});
...
on("monster.recall", function (player, monName) {
  if (player && player.gmDegree > 0) {
    GameServer.broadcast(
      "[Boss] " + monName + " has been summoned by GM " + player.name + "!",
    );
  }
  return true;
});
...
on("animus.maxHP", function (p, v) {
  return v * 1.5;
});
on("animus.attackDP", function (p, v) {
  return v * 1.5;
});
```
...
on("tower.maxHP", function (p, v) {
  return v * 2;
});
on("tower.attackDP", function (p, v) {
  return v * 2;
});
```
...
on("combat.calcGenAtt", function (p, v) {
  return v * 1.1;
});
